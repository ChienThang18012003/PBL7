...
# ========= Build FAISS Index =========
from datetime import datetime  # nếu chưa thêm

def build_job_index():
    global job_index, job_id_list
    # Chỉ lấy các job hợp lệ
    filter_query = {
        "is_deleted": False,
        "status": "Đã duyệt",
    ...
    if vectors:  # kiểm tra nếu có job hợp lệ
...
def find_jobs_by_resume(data: ResumeQuery):
    ...
    location = db["locations"].find_one({"city_id": resume.get("city_id")})
    location_ids = []
    if location:
        # Nếu nhiều location có cùng city_id
        location_ids = [loc["_id"] for loc in db["locations"].find({"city_id": resume.get("city_id")})]

    # Lọc các job có cùng city_id và career_id
    filter_query = {
...
def find_resumes_by_job(data: JobQuery):
    ...
    # Truy vấn thêm location để lấy city_id
