...
const get_All_Resume_Applied_By_Email = async (email, role) => {
    try {
...
const add_Resume_Applied = async (resume_id, job_post_id, email, employer_id) => {
    try {
...
const count_Resume_Applied_By_User = async (email) => {
    try {
        const res = await client.post('/resume-applied/count-resume-applied-by-user', {
            email
        });
        return res.data;
    } catch (error) {
...
const statistic_Resume_Applied_By_Status = async (email, start_date, end_date) => {
    try {
        const res = await client.post('/resume-applied/statistic-resume-applied-by-status-with-email', {
            email,
            start_date,
            end_date
        });
...
const statistic_Resume_Applied_By_Date = async (email, start_date, end_date) => {
    try {
        const res = await client.post('/resume-applied/statistic-resume-applied-by-date', {
            email,
            start_date,
            end_date
        });
...
const get_Distict_Resume_Applied = async (email, role) => {
    try {
        const res = await client.post('/resume-applied/get-distict-resume-applied', {
            email,
            role
        });
...
export default {
    ...
    count_Resume_Applied_By_User,
    statistic_Resume_Applied_By_Date,
    statistic_Resume_Applied_By_Status,
    get_Distict_Resume_Applied
};
