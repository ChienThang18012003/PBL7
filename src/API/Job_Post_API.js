...
const filter_Job_Posts_List = async (city_id, career_id) => {
    try {
        const requestBody = {};
...
const count_Job_Post = async() => {
    try {
        const res = await client.post('job-post/count-job-post')
        return res.data;
    } catch (error) {
        if (error.response) console.log('Error response: ', error.response.data.error);
        else console.log('Error not response: ', error.message);
...
const statistic_Job_Post_By_Status = async(start_date, end_date) => {
    try {
        const res = await client.post('job-post/statistic-job-post-by-status',
            {
                start_date,
                end_date
            }
