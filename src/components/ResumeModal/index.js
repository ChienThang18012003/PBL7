...
export default function ResumeModal({children , data, onSubmitModal, type="confirm", userInfo, email, resumeType = 'default'}) {
  ...
  const handleSubmitModal = async() => {
    if (!desired_position) {
      alert('Bạn chưa nhập vị trí ứng tuyển!');
      return;
    } else if (!desired_job_level) {
      alert('Bạn chưa nhập cấp bậc mong muốn!');
      return;
    } else if (!experience) {
      alert('Bạn chưa nhập kinh nghiệm làm việc!');
      return;
    } else if (!academic_level) {
      alert('Bạn chưa nhập trình độ học vấn!');
      return;
      ...
      alert('Bạn chưa nhập mục tiêu nghề nghiệp!');
      return;
    } else {
        if (type==="confirm"){
          if (resumeType === 'default') {
            const cityInfo = splitTwoPartString(selectedCity);
            const careerInfo = splitTwoPartString(selectedCareer);
            ...
            const careerInfo = splitTwoPartString(selectedCareer);
            if (file) {
              const uploadedFile = await uploadAttachedFile(file, data?.resumeID);
            }
            const updatedResume = await updateResume(data?.resumeID, desired_position, desired_job_level, experience, academic_level, type_of_workplace, job_type, salary_min, salary_max, cityInfo?.first, careerInfo?.first, career_goal);
            onSubmitModal(updatedResume);
            setModal(false);
          }
      } else if (type==="add"){
          if (!file) {
