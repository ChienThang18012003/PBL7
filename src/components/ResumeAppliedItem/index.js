...
import { faHeart as faHeartSolid, faStar, faTrash, faEye } from '@fortawesome/free-solid-svg-icons';
...
function ResumeAppliedItem({ data, children, onDeleteResume, type="confirm", onUpdateStatus }) {
    ...
    useEffect(()=>{
        const update_Resume_Applied = async() => {
            if (selectedStatus !== originalStatus) {
                ...
                console.log(resume);
                if (resume) {setOriginalStatus(selectedStatus); onUpdateStatus(resume)}
            }
        }
    ...
    return (
        <div className={cx('wrapper')}>
            ...
            <div className={cx('candidate-name')}>
                <span>{languageData?.user_id?.username || 'Chưa xác định'}</span>
            </div>
