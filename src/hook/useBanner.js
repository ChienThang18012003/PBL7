...
const useBanner = () => {
    ...
    const getAllBannerByType = async (type) => {
        isBannerLoading(true);
        try {
            const newBanner = await Banner_API.get_All_Banner_By_Type(type);
            return newBanner;
        } catch (error) {
            console.error('Failed to get Banner:', error);
            return null;
    ...
    return [bannerLoading, bannerHook, getAllBanner, addBanner, updateBanner, deleteBanner, getAllBannerByType];
};
