...
const useLocate = () => {
    ...
    const deleteLocation = async (location_Ids) => {
        isLocationLoading(true);
        try {
            const newLocation = await Location_API.delete_Location(location_Ids);
            return newLocation;
        } catch (error) {
    ...
    return [locationLoading, locationHook, getAllLocations, addLocation, changeLocation, deleteLocation];
};
