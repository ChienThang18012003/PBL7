...
const useResumeApplied = () => {
    ...
    const getResumeAppliedByEmail = async (email) => {
        isResumeAppliedLoading(true);
        try {
            const resume = await Resume_Applied_API.get_All_Resume_Applied_By_Email(email);
            return resume;
        } catch (error) {
    ...
    const addResumeApplied= async (resume_id, job_post_id, email) => {
        isResumeAppliedLoading(true);
        try {
            const resume = await Resume_Applied_API.add_Resume_Applied(resume_id, job_post_id, email);
            return resume;
        } catch (error) {
    ...
    const countResumeApplied = async () => {
        isResumeAppliedLoading(true);
        try {
            const resume = await Resume_Applied_API.count_Resume_Applied();
            return resume;
        } catch (error) {
            console.error('Failed to delete resume applied:', error);
            return null;
    ...
    return [resumeAppliedLoading, resumeAppliedHook, getResumeAppliedByEmail, getSpecificResumeApplied, addResumeApplied, deleteResumeApplied, updateResumeApplied, countResumeApplied, statisticResumeAppliedByStatus];
};
