...
import styles from './AdminLocation.module.scss';
...
function AdminLocation() {
    ...
    useEffect(()=>{
        const fetchAllLocation = async() => {
            ...
            if (allLocation && Array.isArray(allLocation)) {
                setDisplayedJobPost(allLocation);
    ...
    useEffect(()=>{
        if (selectedCity === 'all') {
            ...
            setDisplayedDistrict((districtHook || []).filter(district => district?.city_id?.name === selectedCity));
        }
    }, [selectedCity]);
    ...
    useEffect(()=>{
        if (selectedCity === 'all' && selectedDistrict === 'all') {
            ...
            const location = (allLocation || []).filter(post=>post?.city_id?.name === selectedCity);
            setDisplayedJobPost(location);
            setCurrentPage(1);
        } else if (selectedCity === 'all' && selectedDistrict !== 'all') {
            const location = (allLocation || []).filter(post=>post?.district_id?.name === selectedDistrict);
            setDisplayedJobPost(location);
            setCurrentPage(1);
        } else if (selectedCity !== 'all' && selectedDistrict !== 'all') {
            const location = (allLocation || []).filter(post=>post?.district_id?.name === selectedDistrict && post?.city_id?.name === selectedCity);
            setDisplayedJobPost(location);
            setCurrentPage(1);
        }
    ...
    const handleSearch = () => {
        ...
        if (searchValue.trim() !== '') {
            filtered = filtered.filter(post =>
                post?.address?.toLowerCase().includes(searchValue.toLowerCase())
            );
        }
    ...
    const handleAddLocation = (submittedObject) =>{
        if (!submittedObject) return;

        setDisplayedJobPost((prev) => [...prev, submittedObject]);
        setAllLocation((prev) => [...prev, submittedObject]);
    }

    const handleUpdateLocation = (submittedObject) => {
    ...
    const handleDeleteLocation = async () => {
        const deletedLocation = await deleteLocation(selectedLocationIds);
    ...
    if (cityLoading || districtLoading || locationLoading) {
            ...
            <div className={cx('page-content')}>
                <div className={cx('job-post-wrapper')}>
                    ...
                    <div className={cx('filter-wrapper')}>
                        ...
                        <input className={cx('search-box')} placeholder='Nhập địa chỉ' type='text' value={searchValue} onChange={(e)=>{setSearchValue(e.target.value)}}></input>
                        ...
                        <select className={cx('select')} value={selectedDistrict} onChange={(e)=>{setSelectedDistrict(e.target.value)}}>
                            <option key='0' value='all'>
                                --Quận/Huyện--
                            </option>
                            {
                        ...
                        <button className={cx('reload-button')} onClick={()=>{setDisplayedJobPost(allLocation); setSearchValue(''); setSelectedCity('all'); setSelectedDistrict('all');}}>
                    ...
                    <div className={cx('add-button-wrapper')}>
                        <LocationModal type='add' onSubmitModal={handleAddLocation}></LocationModal>
                        <div className={cx('add-button')} onClick={handleDeleteLocation}>
                        ...
                        Array.isArray(currentDoctors) && currentDoctors.map((company,index)=>(
                            <div className={cx('table-item')} key={index}>
                                ...
                                <div className={cx('posted-date-item')}>
                                    <span>
                                        {company?.district_id?.name || 'Chưa xác định'}
                                    </span>
                                </div>
                                <div className={cx('deadline-item')}>
                                    <span>
                                        {company?.address || 'Chưa xác định'}
                                    </span>
                                </div>
                                <div className={cx('view-item')}>
                                    <span>
                                        {company?.lat || 'Chưa xác định'}
                                    </span>
                                </div>
                                <div className={cx('status-item')}>
                                    <span>
                                        {company?.lng || 'Chưa xác định'}
                                    </span>
                                </div>
                                <div className={cx('action-button-wrapper')}>
                                    <LocationModal type='update' data={company} onSubmitModal={handleUpdateLocation}></LocationModal>
                                </div>
                            </div>
                        ))
                    }
