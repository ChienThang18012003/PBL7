...
function AdminMain() {
    ...
    useEffect(()=>{
        if (cityHook) {setDisplayedJobPost(cityHook); setAllCity(cityHook);}
    },[cityHook])
    ...
    const handleAddCity = (submittedObject) =>{
        if (!submittedObject) return;

        setDisplayedJobPost((prev) => [...prev, submittedObject]);
        setAllCity((prev) => [...prev, submittedObject]);
    }

    const handleUpdateCity = (submittedObject) => {
        ...
        setAllCity((prev) =>
            prev.filter((item) => !selectedCityIds.includes(item._id))
        );
    };

    const handleDeleteCity = async () => {
        const deletedCity = await deleteCity(selectedCityIds);
            ...
            <div className={cx('page-content')}>
                <div className={cx('job-post-wrapper')}>
                    ...
                    <div className={cx('filter-wrapper')}>
                        ...
                        <input className={cx('search-box')} placeholder='Nhập tên tỉnh/thành phố' type='text' value={searchValue} onChange={(e)=>{setSearchValue(e.target.value)}}></input>
                        <button className={cx('reload-button')} onClick={()=>{setDisplayedJobPost(allCity); setSearchValue('')}}>
                    ...
                    <div className={cx('add-button-wrapper')}>
                        <CityModal type='add' onSubmitModal={handleAddCity}></CityModal>
                        <div className={cx('add-button')} onClick={handleDeleteCity}>
                    ...
                    <div className={cx('table-container')}>
                    {
                        Array.isArray(currentDoctors) && currentDoctors.map((company,index)=>(
                            <div className={cx('table-item')} key={index}>
                                <input type='checkbox' className={cx('city-checkbox')} checked={selectedCityIds.includes(company?._id)} onChange={(e) => handleCheckboxChange(company?._id, e.target.checked)}></input>
                                ...
                                <div className={cx('action-button-wrapper')}>
                                    <CityModal type='update' data={company} onSubmitModal={handleUpdateCity}></CityModal>
                                </div>
                            </div>
                        ))
                    }
