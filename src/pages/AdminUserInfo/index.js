...
import { faUpload, faBuilding, faBriefcase, faComment, faCaretDown, faCaretUp, faNewspaper, faUser, faSave, faUsers, faCity, faMountainCity, faLocationDot, faFileInvoice, faImage, faChartSimple } from '@fortawesome/free-solid-svg-icons';
...
function AdminUserInfo() {
        ...
        const fetchAccount = async () => {
            if (id) {
                ...
                if (AccountInfo) {
                    ...
                    setOriginalRole(AccountInfo?.role || '')
                    setIsActive(AccountInfo?.is_active)
    ...
    const handleUpdateInfo = async() => {
        ...
        if (role !== originalRole) {
            const newRole = await changeAccountRole(userInfo?.email, role);
            if (newRole) {setOriginalRole(role); toast.success('Cập nhật vai trò thành công')}
        }
            ...
            <div className={cx('page-content')}>
                <div className={cx('account-info-container')}>
                    ...
                    <div className={cx('field-container')}>
                        ...
                        <select type="text" className={cx('field-input')} value={role} onChange={(e)=>{setRole(e.target.value)}}>
                            <option key='0' value='user'>
                                user
                            </option>
                            <option key='1' value='employer'>
                                employer
                            </option>
                    ...
                    <div className={cx('account-detail-container-two')}>
                        <div className={cx('section-one')}>
                            ...
                            <div className={cx('action-info-wrapper')}>
                                <input type='checkbox' className={cx('active-checkbox')} checked={is_active} onClick={async () => {
                                    try {
                                        const updatedAccount = await updateAccountStatus(userInfo?._id, !is_active);
                                        if (updatedAccount) {setIsActive(!is_active); toast.success('Cập nhật trạng thái tài khoản thành công!')}
                                    } catch (error) {
                                        console.error("Error updating account:", error);
                                    }
                                    }}
                                    ></input>
                                <div className={cx('active-text')}>
                                    <span>
                                        is_active
                                    </span>
                                </div>
